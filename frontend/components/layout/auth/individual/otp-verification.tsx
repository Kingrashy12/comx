...
import { createUserIndividual, sendOtp, verifyOTP } from "@/helper/api";
import React, { useState } from "react";

const OTPVerification = () => {
  const { individualForm, setRegState } = useAuthFlow();
  ...
  const verifyOtp = async () => {
    const res = await verifyOTP({ email: individualForm.email, otp: code });
    if (res) {
      if (res?.success) {
        toast.success(res.message);
        const token = await createUserIndividual({ ...individualForm });
        signIn(token);
    ...
    <>
      <Box column className="gap-4" fullWidth>
        <span className="text tracking-wide !leading-snug">
          Enter the 4-digit code that was sent to {individualForm.email}
        </span>
        <TextInput
